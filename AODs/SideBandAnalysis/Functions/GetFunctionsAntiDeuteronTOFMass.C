...
void MakeMassPlotAll(Double_t *parMean,Double_t* parSig,TString name,double yRangeLow,double yRangeHigh) {
 ...
 int npoint = 0;
   for(double pt = 0.8;pt<= 4.05; pt+=0.1){
     ...
     gSigmaUp->SetPoint(npoint,pt,GetDeuteronMass2Mean_pp(pt)+2.0*GetDeuteronMass2Sigma_pp(pt));
     gSigmaLow->SetPoint(npoint,pt,GetDeuteronMass2Mean_pp(pt)-2.0*GetDeuteronMass2Sigma_pp(pt));
   /*  gMean->SetPoint(npoint,pt,fMean1(pt));
    ...
    TString pdfMassName = Form("/home/sbhawani/cernbox/ProtonDeuteron/Outputs/AODs/SideBandAnalysis/Functions/PlotForAntiDeuteron%s.pdf", name.Data()); //plots are output as .pdf If you prefer other formats simply change the ending
...
TH2F* MassHisto(const char *PathAnalysisFile, int Addon) {
  ...
  TDirectoryFile *MyTaskDirectory1 = (TDirectoryFile*) (FileData->FindObjectAny(Form("HMAntiDeuteronDCA%i", Addon)));
  TList *MyTaskList= (TList*) (MyTaskDirectory1->FindObjectAny(Form("HMAntiDeuteronDCA%i", Addon)));
...
void CompareMassPlotAll(const char *PathAnalysisFile, int Addon,Double_t *parMean,Double_t* parSig,TString name,double yRangeLow,double yRangeHigh) {
 ...
 int npoint = 0;
   ...
   /*  gMean->SetPoint(npoint,pt,fMean1(pt));
    ...
    TCanvas *Plot = new TCanvas("cPlot", "cPlot", 0, 0, 900, 600);
    TString pdfMassName = Form("/home/sbhawani/cernbox/ProtonDeuteron/Outputs/AODs/SideBandAnalysis/Functions/FinalForAntiDeuteronComparePlotFor%s.pdf", name.Data()); //plots are output as .pdf If you prefer other formats simply change the ending
    Plot->SetLeftMargin(0.09);
    Plot->SetRightMargin(0.035);
    Plot->SetTopMargin(0.9505);
    Plot->SetBottomMargin(0.135);
   ...
   // gMean->SetTitleSize(0.05);
    ...
    hMassTPCTOF->GetXaxis()->SetTitleSize(0.05);
    hMassTPCTOF->GetXaxis()->SetTitleOffset(0.89);
    hMassTPCTOF->GetXaxis()->SetTitle("p_{T} (GeV/#it{c})");
    hMassTPCTOF->GetYaxis()->SetLabelSize(0.035);
    ...
    hMassTPCTOF->GetXaxis()->SetRangeUser(0.0, 5.05);
...
void GetFunctionsAntiDeuteronTOFMass() {
 ...
 // MakeMassPlotAll(parMean,parSig,"AllFunctions",2.5,4.5);
  CompareMassPlotAll(FileNameData0,0,parMean,parSig,"AllFunctions",0.0,6);
  return;
}
